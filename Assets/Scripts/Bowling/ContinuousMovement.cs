...
public class ContinuousMovement : MonoBehaviour
{
    ...
    {
        character.height = rig.CameraInOriginSpaceHeight + additionalHeight; // VR Camera의 높이와 캐릭터의 높이 동기화
        Vector3 capsuleCenter = transform.InverseTransformPoint(rig.Camera.transform.position); // VR Camera의 World 좌표를 Local 좌표로 변환하여 저장함.
        character.center = new Vector3(capsuleCenter.x, character.height / 2 + character.skinWidth, capsuleCenter.z); // VR Camera의 높이와 위치 좌표를 이용해서 캐릭터의 Center를 재설정함.
    }
